...
| 费用估算 | 弹框 · 费用 Tab | 最近一次提问 / 本会话 / 今日·本项目 / 今日·全部 四卡；每张卡内**按 API Key 分行**展示各自 token 四桶与费用（token 不区分官方与否，一律统计；**费用仅官方 Key（api.deepseek.com）计算**，非官方标注「不计费」）；卡片头部保留合计金额 + 叠加柱状图 |
| 价格设置 | 弹框 · 价格 Tab | **官方价格表式排版**；模型 × 高峰/空闲双时段单价；时段窗口 + 时区滑块可配置；旧配置自动迁移为官方 V4 三档 |
| 会话头部按钮 | 会话头部 utilities | 实时「**当前会话 ≈xx CNY**」（金额绿色）；点击即刷新 |
...
| 时段 | 默认北京 9:00–12:00、14:00–18:00 为高峰，其余为空闲；空闲 = 高峰 × 0.5 |
