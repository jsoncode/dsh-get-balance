...
*/
function parseLogFile(path, isZstd) {
	...
	while (cursor < text.length) {
		...
		if (parsed.type === "step/end") {
			if (typeof parsed.time === "number" && Number.isFinite(parsed.time)) sample.steps.push({
				time: parsed.time,
...
*/
function computeLive(range, now, offsetMs, root, config, providerBaseUrls) {
