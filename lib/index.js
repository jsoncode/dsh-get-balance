...
function normalizePriceConfig(raw) {
	...
	if (raw !== null && typeof raw === "object") {
		...
		if (isLegacyDefaultTiers(tiers)) return fallback();
		const offset = numberOr$1(obj["timezoneOffsetMinutes"], 480);
...
function foldSessionEvents(events) {
	...
	for (const event of events) {
		...
		const turn = numberOr$1(data.turn);
		const step = numberOr$1(data.step);
...
function parseTodayFile(path, isZstd, todayStart, config, providerBaseUrls) {
	...
	for (const line of text.split("\n")) {
		...
		addBuckets$1(peak ? provider.peak : provider.offPeak, usageBuckets);
		if (isOfficialProvider(currentProvider, providerBaseUrls)) {
			const pair = provider.billable.get(model) ?? {
				peak: zeroBuckets$1(),
				offPeak: zeroBuckets$1()
			};
			addBuckets$1(peak ? pair.peak : pair.offPeak, usageBuckets);
...
const seriesFileCache = /* @__PURE__ */ new Map();
const zeroBuckets = () => ({
	uncachedInput: 0,
	cacheRead: 0,
	cacheWrite: 0,
...
*/
function parseFileSteps(path, isZstd) {
	const text = decodeLog(path, isZstd);
	if (text === void 0) return void 0;
	const sample = {
		steps: [],
...
function ensureParsed(ref) {
	...
	if (cached !== void 0 && cached.mtimeMs === stat.mtimeMs && cached.size === stat.size) return cached.sample;
	const sample = parseFileSteps(ref.path, ref.isZstd);
	if (sample !== void 0) seriesFileCache.set(ref.path, {
		mtimeMs: stat.mtimeMs,
		size: stat.size,
		sample
	});
...
*/
async function computeSeries(range, config, providerBaseUrls, rootOverride) {
	const now = Date.now();
