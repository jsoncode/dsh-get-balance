...
async function queryBalances(entries, refresh) {
	...
	return Promise.all(entries.map(async (entry) => {
		if (!refresh) {
			const cached = cache$1.get(entry.id);
			if (cached !== void 0 && now - cached.at < CACHE_TTL_MS) return cached.result;
		}
		const result = await fetchOne(entry);
		if (result.ok || result.code === "auth-failed" || result.code === "no-credential") cache$1.set(entry.id, {
			at: now,
			result
		});
		return result;
	...
	try {
		switch (request.op) {
			...
			case "cost": {
				...
				return {
					ok: true,
					cost: await computeCosts(sessionId, deps.sessions, readPriceConfig(deps), process.cwd(), providerBaseUrls)
				};
			}
