 ...
 * 2. 费用：筛选器（API Key / 平台 / 模型 / 时间）+ 四张 ECharts 图
 *    （费用+Token 组合图 / 工作区 / 缓存比例 / 工具占比），见 CostTab.tsx；
...
function validateTiers(list) {
    if (list.length === 0)
        return t('pricesEmpty');
    const seen = new Set();
    for (const tier of list) {
        const match = tier.match.trim();
        if (match.length === 0)
            return t('modelMatchRequired');
        if (seen.has(match))
            return t('modelMatchDuplicate');
        seen.add(match);
    }
    return null;
}
...
export function BalanceModal({ run, useOpen, close, getSession, useTick, useAutoSeconds, setAutoSeconds, bumpPriceTick, useShowBalance, setShowBalance }) {
    ...
    const [modelBackup, setModelBackup] = useState(null);
    ...
    return (_jsxs("div", { className: "dshb-backdrop", onClick: (e) => { if (e.target === e.currentTarget)
            close(); }, children: [_jsxs("div", { className: "dshb-modal", role: "dialog", "aria-modal": "true", children: [_jsxs("div", { className: "dshb-modal-header", children: [_jsx("div", { className: "dshb-modal-title", children: t('modalTitle') }), _jsxs("div", { className: "dshb-tabs", children: [_jsx("button", { type: "button", className: 'dshb-tab' + (tab === 'balance' ? ' dshb-tab-active' : ''), onClick: () => setTab('balance'), children: t('tabBalance') }), _jsx("button", { type: "button", className: 'dshb-tab' + (tab === 'cost' ? ' dshb-tab-active' : ''), onClick: () => setTab('cost'), children: t('tabCost') }), _jsx("button", { type: "button", className: 'dshb-tab' + (tab === 'prices' ? ' dshb-tab-active' : ''), onClick: () => setTab('prices'), children: t('tabPrices') })] }), _jsxs("div", { className: "dshb-head-ops", children: [_jsxs("button", { type: "button", className: "dshb-btn dshb-btn-small", onClick: openTimingDialog, children: [t('timingBtn'), autoSeconds > 0 ? '·' + autoSeconds + 's' : ''] }), tab === 'balance'
                ...
                : null, modelDialogOpen
                ? (_jsx("div", { className: "dshb-timing-backdrop", onClick: (e) => { if (e.target === e.currentTarget)
                        cancelModelDialog(); }, children: _jsxs("div", { className: "dshb-model-dialog", role: "dialog", "aria-modal": "true", "aria-label": t('modelDialogTitle'), children: [_jsx("div", { className: "dshb-timing-title", children: t('modelDialogTitle') }), _jsx("p", { className: "dshb-hint", children: t('modelDialogHint') }), _jsx("div", { className: "dshb-model-scroll", children: _jsxs("table", { className: "dshb-table dshb-model-table", children: [_jsxs("thead", { children: [_jsxs("tr", { children: [_jsx("th", { rowSpan: 2, className: "dshb-model-col-name", children: t('priceName') }), _jsx("th", { rowSpan: 2, className: "dshb-model-col-match", children: t('priceMatch') }), _jsx("th", { colSpan: 3, className: "dshb-period-off", children: t('pricePeriodOffPeak') }), _jsx("th", { colSpan: 3, className: "dshb-period-peak dshb-model-sep", children: t('pricePeriodPeak') }), _jsx("th", { rowSpan: 2, className: "dshb-model-col-ops", children: t('priceOps') })] }), _jsx("tr", { children: ['offPeak', 'peak'].map((period) => (['input', 'cacheRead', 'output'].map((field, k) => (_jsx("th", { className: period === 'peak' && k === 0 ? 'dshb-model-sep' : undefined, children: field === 'input' ? t('priceInput') : field === 'cacheRead' ? t('priceCacheRead') : t('rateOutput') }, period + field))))) })] }), _jsx("tbody", { children: (prices ?? []).map((tier, i) => (_jsxs("tr", { children: [_jsx("td", { children: _jsx("input", { className: "dshb-input", value: tier.name, placeholder: t('priceName'), "aria-label": t('priceName'), onChange: (e) => updatePrice(i, { name: e.target.value }) }) }), _jsx("td", { children: _jsx("input", { className: "dshb-input", value: tier.match, placeholder: t('priceMatch'), "aria-label": t('priceMatch'), title: tier.match === '*' ? t('fallbackHint') : t('modelMatchHint'), onChange: (e) => updatePrice(i, { match: e.target.value }) }) }), ['offPeak', 'peak'].map((period) => (['input', 'cacheRead', 'output'].map((field, k) => (_jsx("td", { className: period === 'peak' && k === 0 ? 'dshb-model-sep' : undefined, children: _jsx("input", { className: "dshb-input dshb-num", type: "number", step: "any", min: 0, value: tier[period][field], "aria-label": t(period === 'peak' ? 'pricePeriodPeak' : 'pricePeriodOffPeak') + ' · ' + (field === 'input' ? t('priceInput') : field === 'cacheRead' ? t('priceCacheRead') : t('rateOutput')), onChange: (e) => updateRate(i, period, field, Number(e.target.value) || 0) }) }, period + field))))), _jsx("td", { className: "dshb-model-ops", children: _jsx("button", { type: "button", className: "dshb-btn dshb-btn-small dshb-btn-danger", title: t('deleteModel'), "aria-label": t('deleteModel'), onClick: () => removeTier(i), children: t('delete') }) })] }, tier.id))) })] }) }), priceMsg !== ''
                                ? _jsx("p", { className: "dshb-err", children: priceMsg })
                                : null, _jsxs("div", { className: "dshb-model-actions", children: [_jsxs("button", { type: "button", className: "dshb-btn dshb-btn-small dshb-model-add", onClick: addTier, children: ["+ ", t('addModel')] }), _jsx("button", { type: "button", className: "dshb-btn dshb-btn-small", onClick: cancelModelDialog, children: t('cancel') }), _jsx("button", { type: "button", className: "dshb-btn dshb-btn-small dshb-btn-primary", onClick: confirmModelDialog, children: t('ok') })] })] }) }))
                : null] }));
}
