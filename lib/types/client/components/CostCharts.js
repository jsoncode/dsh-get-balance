...
import { fmtAmount, fmtTokens } from "../i18n.js";
...
export function costTooltip(params, currency, notPricedName) {
    const rows = params;
    const axis = rows[0]?.axisValue;
    let html = '<div style="font-weight:600;margin-bottom:4px">' + String(axis ?? '') + '</div>';
    for (const p of rows) {
 ...
 */
export function ChartCard({ title, option, active, wide }) {
    ...
    return (_jsxs("div", { className: 'dshb-chart' + (wide === true ? ' dshb-chart-wide' : ''), children: [_jsx("div", { className: "dshb-chart-title", children: title }), _jsx("div", { className: "dshb-chart-box", ref: boxRef })] }));
}
