...
import { currencySymbol, fmtAmount, fmtCompact, fmtTokens, getLang, t } from "../i18n.js";
...
/** 堆叠柱状图公共 option 骨架。 */
export function stackedBarOption(labels, series, yName, tooltip) {
    ...
    return {
        ...
        yAxis: {
            ...
            axisLabel: { color: cssVar('--dsw-alias-label-tertiary', '#999'), fontSize: 10, formatter: (v) => fmtCompact(v) },
        },
...
function defaultTokenTooltip(params) {
    const rows = params;
 ...
 */
export function costTokensComboOption(labels, series, yLeftName, yRightName, currency) {
    const axisColor = cssVar('--dsw-alias-label-secondary', '#888');
    const gridColor = cssVar('--dsw-alias-border-l1', '#eee');
    const labelPrimary = cssVar('--dsw-alias-label-primary', '#222');
    const tooltipBg = cssVar('--dsw-alias-bg-layer-2', '#fff');
    ...
    return {
        tooltip: {
            ...
            borderColor: cssVar('--dsw-alias-border-l2', '#ddd'),
            textStyle: { color: labelPrimary, fontSize: 12 },
            formatter: (params) => comboTooltip(params, currency),
            ...TOOLTIP_CONFINE,
        },
        legend: {
            // plain 模式（缺省）：图例按宽度自动换行，不再滚动翻页。
...
function comboTooltip(params, currency) {
    const rows = params;
    const axis = rows[0]?.axisValue;
    let html = '<div style="font-weight:600;margin-bottom:4px">' + String(axis ?? '') + '</div>';
    for (const p of rows) {
...
export function cacheTooltip(params) {
    const rows = params;
    const axis = rows[0]?.axisValue;
 ...
 */
export function ChartCard({ title, option, active }) {
    ...
    return (_jsxs("div", { className: "dshb-chart", children: [_jsx("div", { className: "dshb-chart-title", children: title }), _jsx("div", { className: "dshb-chart-box", ref: boxRef })] }));
}
