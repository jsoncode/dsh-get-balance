...
export function HeaderButton({ sessionId, run, useTick, usePriceTick, useSession, useChat, subscribeSessionEvents, bumpBalanceTick }) {
    const tick = useTick();
    ...
    const assistantSeq = useChat ? useChat((s) => maxAssistantSeqOf(s?.legacy?.nodes)) : 0;
    ...
    const shownSessionIdRef = useRef(null);
    // 上一次观察到的 assistant seq：null = 尚未观察（首次只记录，不触发）。
    const prevAssistantSeq = useRef(null);
        ...
        const kick = () => {
            void refresh().then((applied) => {
                ...
                if (cancelled || sessionIdRef.current !== sessionId)
                    return;
                if (applied || attempts >= SWITCH_REFRESH_RETRIES)
                    return;
                attempts += 1;
    ...
    const scheduleRefresh = useCallback((gate) => {
        if (gate)
            gateRef.current = true;
        if (coalesceTimerRef.current !== null)
            return;
        const wait = Math.max(REFRESH_COALESCE_MS, lastRefreshAtRef.current + REFRESH_MIN_INTERVAL_MS - Date.now());
    ...
    useEffect(() => {
        if (subscribeSessionEvents === undefined)
            return;
        let dispose = null;
        let timer = null;
        let attempts = 0;
