...
const COPY = {
    zh: {
        ...
        seriesError: '加载失败',
        retry: '重试',
        costTabHint: '金额仅对官方 Key（api.deepseek.com）且已配置定价的模型计费；未配置定价的平台/模型归入「未计费」层（柱高为 token 量示意，不计费）。筛选（API Key / 平台 / 模型）在本地即时生效；时间切换重新统计。',
        // 价格 tab
    ...
    en: {
        ...
        retry: 'Retry',
        costTabHint: 'Only official keys (api.deepseek.com) with a configured price tier are billed; unpriced platforms/models go into the "Not priced" layer (bar height is a token-volume indicator, not billed). Filtering (API Key / Platform / Model) applies locally; switching time range re-queries the host.',
        // prices tab
