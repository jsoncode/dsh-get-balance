...
const COPY = {
    zh: {
        ...
        seriesError: '加载失败',
        retry: '重试',
        costTabHint: '费用仅对官方 Key（api.deepseek.com）且已配置定价的模型计费；未配置定价的记录不计费，不在费用图中显示（Token 使用量图仍统计全部模型）。筛选按 平台 → API Key → 模型 逐级关联，本地即时生效；时间切换重新统计。',
    ...
    en: {
        ...
        retry: 'Retry',
        costTabHint: 'Only official keys (api.deepseek.com) with a configured price tier are billed; unpriced records are not billed and do not appear in the cost chart (the token-usage chart still covers all models). Filters cascade Platform → API Key → Model and apply locally; switching time range re-queries the host.',
