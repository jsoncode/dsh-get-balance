...
const COPY = {
    zh: {
        ...
        showInMenu: '在菜单中显示',
        showInMenuDesc: '开启后，在宿主侧栏底部显示「余额」入口按钮',
        openPlugin: '打开余额面板',
        tabBalance: '余额',
        tabCost: '费用',
        tabPrices: '价格设置',
        ...
        seriesError: '加载失败',
        retry: '重试',
        costTabHint: '费用仅对官方 Key（api.deepseek.com）且已配置定价的模型计费：组合图中柱状为各模型费用（仅计费模型，左轴为金额），折线为各模型 Token 用量（含未计费模型，右轴为 Token 数）。筛选按 平台 → API Key → 模型 逐级关联，本地即时生效；时间切换重新统计。',
        backfillConfirmTitle: '首次加载历史数据',
        backfillConfirmText: '首次查看该时间范围需要解析宿主中的历史会话日志（约 {size}），可能需要几十秒到一两分钟。首次计算后数据会永久保存，之后秒开。确认开始？',
        backfillConfirmBtn: '确认开始',
        backfillLoading: '正在加载历史数据（首次计算），请耐心等待…',
        backfillLoadingElapsed: '已等待 {s} 秒',
        backfillCancelHint: '取消后该时间范围本次不加载，可稍后切换其他范围再试。',
        // 价格 tab
        ...
        deleteModel: '删除该模型',
        editModels: '编辑模型',
        modelDialogTitle: '编辑模型价格',
        modelDialogHint: '匹配：模型 id 精确匹配，或以 * 作为兜底档；名称为空时用匹配串。单价单位：元 / 百万 tokens。',
        rateOutput: '输出',
        ok: '确定',
        modelMatchHint: '匹配：模型 id 精确匹配，或以 * 作为兜底档；新增档默认复制首档单价。',
        modelMatchRequired: '请填写模型匹配（模型 id 或 *）',
        modelMatchDuplicate: '模型匹配重复：每个价格档的匹配串需唯一',
    ...
    en: {
        ...
        showInMenuDesc: 'When on, a Balance entry button appears at the bottom of the host sidebar',
        openPlugin: 'Open balance panel',
        tabBalance: 'Balance',
        tabCost: 'Cost',
        tabPrices: 'Prices',
        tabPlatformDeepseek: 'DeepSeek',
        ...
        retry: 'Retry',
        costTabHint: 'Only official keys (api.deepseek.com) with a configured price tier are billed. In the combined chart, bars show per-model cost (billed models only; left axis is amount) and lines show per-model token usage (including unpriced models; right axis is token count). Filters cascade Platform → API Key → Model and apply locally; switching time range re-queries the host.',
        backfillConfirmTitle: 'First-time history load',
        backfillConfirmText: 'Viewing this range for the first time requires parsing the host\'s historical session logs (about {size}); this may take tens of seconds to a minute or two. The computed data is then saved permanently, and later loads are instant. Start now?',
        backfillConfirmBtn: 'Start',
        backfillLoading: 'Loading history (first-time computation), please wait…',
        backfillLoadingElapsed: 'Waiting for {s} s',
        backfillCancelHint: 'After cancelling, this range won\'t load this time; you can try other ranges or retry later.',
        // prices tab
        ...
        priceInputHit: '1M input (cache)',
        priceInputMiss: '1M input',
        priceOutput: '1M output',
        ...
        modelDialogTitle: 'Edit model prices',
        modelDialogHint: 'Match: an exact model id, or * as the fallback tier; an empty name falls back to the match string. Rates are CNY per million tokens.',
        rateOutput: 'Output',
        ok: 'OK',
        modelMatchHint: 'Match: an exact model id, or * as the fallback tier. A new tier copies the first tier rates.',
        modelMatchRequired: 'Model match is required (model id or *)',
        modelMatchDuplicate: 'Duplicate model match: every tier needs a unique match string',
...
export function zhNumeral(index) {
    const list = dictOf().cnNum;
    return list !== undefined && index >= 0 && index < list.length ? list[index] : String(index);
}
/** 金额格式化（保留合理小数位）。 */
...
export const fmtCompact = (n) => {
    ...
    for (let k = 0; k < COMPACT_UNITS.length; k++) {
        if (abs >= COMPACT_UNITS[k].div) {
            i = k;
            break;
        }
    }
    let s = strip((n / COMPACT_UNITS[i].div).toFixed(1));
    // 四舍五入进位（999.99B → 1000B）→ 上调到更大单位（1T）。
    while (Math.abs(Number(s)) >= 1000 && i > 0) {
        i -= 1;
        s = strip((n / COMPACT_UNITS[i].div).toFixed(1));
    }
    return s + COMPACT_UNITS[i].suffix;
};
