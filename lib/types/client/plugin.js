...
export function createPlugin() {
    return {
        ...
        apply(ctx) {
            ...
            // 实时比对被安装根目录 package.json（无时间缓存），hasUpdate=true 时
            // footer 按钮最右侧显示【更新】胶囊。每次页面刷新触发一次检查；
            // 失败静默（不显示胶囊）。
            // recheckUpdate 在更新进程成功结束后再次调用（宿主实时重读版本），
            // 用于让「更新」胶囊立即消失。
            const recheckUpdate = () => {
            ...
            const subscribeSessionEvents = (sessionId, onAssistantMessage) => {
            ...
            slots.inject(FOOTER_SLOT, () => slots.register({ name: FOOTER_SLOT, id: FOOTER_ENTRY_ID, order: 30 }, (props) => (_jsx(FooterButton, { onOpen: () => modalStore.open(true), reportSession: (s) => { if (s)
                    sessionRef.current = s; }, wide: props.wide, useSessions: props.useSessions, run: run, useOpen: useOpen, usePriceTick: usePriceTick, useBalanceTick: useBalanceTick, useShowBalance: useShowBalance, useUpdate: useUpdate, onUpdateClick: openUpdateConfirm }))));
            ...
            slots.inject('settings.section', () => slots.register({
                ...
                order: 42,
                label: () => t('balanceBtn'),
            }, (props) => (_jsx(PluginSettingsPage, { onOpen: () => modalStore.open(true), close: typeof props.close === 'function' ? props.close : () => { } }))));
            // ─── 会话头部工具区：当前会话费用 / 余额按钮（header.utilities）────
            slots.inject('conversation.session.header.utilities', () => slots.register({ name: 'conversation.session.header.utilities', id: 'dsh-balance-header', order: 100 }, (props) => (_jsx(HeaderButton, { sessionId: String(props.sessionId ?? ''), run: run, useTick: useTick, usePriceTick: usePriceTick, useSession: props.useSession, useChat: props.useChat, subscribeSessionEvents: subscribeSessionEvents, bumpBalanceTick: bumpBalanceTick }))));
            // ─── 统一「余额」弹框（余额 / 费用 / 价格设置 三 tab）──────────────
            slots.inject('shell.overlay', () => slots.register({ name: 'shell.overlay', id: 'dsh-balance-modal' }, () => (_jsx(BalanceModal, { run: run, useOpen: useOpen, close: () => modalStore.close(), getSession: getSession, useTick: useTick, useAutoSeconds: useAutoSeconds, bumpPriceTick: bumpPriceTick, useShowBalance: useShowBalance, setShowBalance: setShowBalance, setAutoSeconds: (seconds) => {
