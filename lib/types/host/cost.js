...
import { SESSION_LOG_CANDIDATES, decodeLog, getParsedFile } from "./log-cache.js";
...
function upgradeLegacyDefaultTiers(tiers) {
    return tiers.map((tier) => {
        ...
        if (legacy === undefined)
            return tier;
        if (!samePeriodPrices(tier.peak, legacy.peak) || !samePeriodPrices(tier.offPeak, legacy.offPeak))
            return tier;
