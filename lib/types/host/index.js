...
import { initConfigFile } from "./config-file.js";
    ...
    if (commands !== undefined) {
        commands.register({
            ...
            description: 'DeepSeek 余额与费用查询：列出服务商、查询官方余额、计算费用（最近一次提问/本会话/今日·本项目/今日·全部）、管理价格档与附加 API key。Query DeepSeek balances and token costs. 参数为 JSON：'
                + '{ "op": "providers|balance|cost|pricesGet|pricesSave|keysGet|keysSave|autoRefreshGet|autoRefreshSave|updateCheck|pluginUpdateStart|pluginUpdateStatus", "sessionId": "...", "refresh": true, ... }。',
