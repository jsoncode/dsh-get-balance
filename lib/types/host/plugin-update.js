...
export function startPluginUpdate() {
    ...
    const rec = { child, output: '', running: true, exitCode: null, error: '', startedAt: Date.now(), finishedAt: null };
    run = rec;
...
export function getPluginUpdateStatus() {
    if (run === null)
        return { running: false, done: false, output: '', exitCode: null, error: '', startedAt: null, finishedAt: null };
    return {
        running: run.running,
        done: !run.running,
        output: run.output,
