...
import { costOf, decodeLog, isOfficialProvider, periodPricesOf } from "./cost.js";
 ...
 */
export async function computeSeries(range, config, providerBaseUrls, rootOverride) {
    const now = Date.now();
    ...
    if (range !== 'all') {
        ...
        if (range === 'hour1')
            coarseStart = now - 60 * MIN10;
        else if (range === 'today')
            coarseStart = todayStart;
        else if (range === 'week7')
            coarseStart = todayStart - 6 * DAY;
        else if (range === 'month1')
            coarseStart = todayStart - 29 * DAY;
    }
    ...
    return { range: range, bucket: axis.bucket, points, records, currency };
}
