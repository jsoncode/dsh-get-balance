/**
 * dsh-get-balance —— 浏览器半边：本地偏好（localStorage 持久化的视图状态）。
 *
 * 目前只有「在菜单中显示」一个开关：关闭后宿主侧栏 footerAction 的
 * 【余额】入口按钮渲染 null（不占位、不轮询余额）。纯客户端视图偏好，走
 * localStorage 即可（余额 / 费用 / 价格等数据仍由宿主半边持有）。
 *
 * 默认**开启**：侧栏入口是本插件既有的入口，升级后不应凭空消失；用户可在
 * 宿主「设置 → 账户余额 · Token 调用量」分区页，或插件弹框「价格设置」tab
 * 顶部把它关掉。两处渲染同一个 showInMenuStore，改一处另一处即时同步。
 */

/** localStorage 键。 */
const STORAGE_KEY = 'dsh-get-balance.show-in-menu'
