...
export const css = [
  ...
  // 首次回填确认后的全量 loading 遮罩（覆盖费用 tab 内容区，模态框头/关闭按钮仍可点）
  '.dshb-cost-root{position:relative}',
  '.dshb-backfill-overlay{position:absolute;inset:0;z-index:60;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;text-align:center;padding:24px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-1,#fff) 76%,transparent);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);border-radius:10px;color:var(--dsw-alias-label-primary,#222);font-size:14px;pointer-events:auto}',
