...
export const DEFAULT_PEAK_WINDOWS: readonly TimeWindow[] = [
...
export const DEFAULT_PRICES: PriceTier[] = [
  ...
  {
    id: 'deepseek-v4.1-flash-expires-on-0910', name: 'deepseek-v4.1-flash-expires-on-0910', currency: 'CNY', match: 'deepseek-v4.1-flash-expires-on-0910',
    peak: { input: 2.0, cacheRead: 0.04, cacheWrite: 0, output: 8.0 },
    offPeak: { input: 1.0, cacheRead: 0.02, cacheWrite: 0, output: 4.0 },
  },
  {
...
export interface SessionEventLike {
  ...
  data?: {
    ...
    provider?: string
    chunk?: { type?: string; usage?: UsageLike }
  }
}
...
const numberOrZero = (v: unknown): number => (typeof v === 'number' && Number.isFinite(v) && v >= 0 ? v : 0)

function normalizePeriod(raw: unknown): PricePeriodPrices {
...
function normalizeTier(raw: unknown, index: number): PriceTier {
  const obj = (raw !== null && typeof raw === 'object' ? raw : {}) as Record<string, unknown>
  ...
  const id = typeof obj['id'] === 'string' && (obj['id'] as string).length > 0 ? obj['id'] as string : 'tier-' + index
  return {
    id,
    name: typeof obj['name'] === 'string' ? obj['name'] as string : id,
    currency: typeof obj['currency'] === 'string' && (obj['currency'] as string).length > 0 ? obj['currency'] as string : 'CNY',
    match: typeof obj['match'] === 'string' && (obj['match'] as string).length > 0 ? obj['match'] as string : '*',
    peak,
    offPeak,
  }
}
 ...
 */
export function isOfficialProvider(provider: string | undefined, providerBaseUrls: Record<string, string>): boolean {
...
function isLegacyDefaultTiers(tiers: readonly PriceTier[]): boolean {
...
export function normalizePriceConfig(raw: unknown): PriceConfig {
  const fallback = (): PriceConfig => JSON.parse(JSON.stringify(DEFAULT_PRICE_CONFIG)) as PriceConfig
  ...
  if (raw !== null && typeof raw === 'object') {
    ...
    const windows: TimeWindow[] = Array.isArray(windowsRaw) && windowsRaw.length > 0
      ? windowsRaw
        .map((w) => {
          const o = (w !== null && typeof w === 'object' ? w : {}) as Record<string, unknown>
          const start = typeof o['start'] === 'string' ? o['start'] as string : ''
          const end = typeof o['end'] === 'string' ? o['end'] as string : ''
...
/** 取某档在指定时刻生效的单价集合。 */
export function periodPricesOf(tier: PriceTier, timeMs: number, config: PriceConfig): PricePeriodPrices {
 ...
 */
export function matchTier(model: string | undefined, prices: readonly PriceTier[]): PriceTier | undefined {
...
function indexOfMagic(raw: Buffer): number {
  for (let i = 0; i + 4 <= raw.length; i++) {
    if (raw[i] === ZSTD_MAGIC[0] && raw[i + 1] === ZSTD_MAGIC[1] && raw[i + 2] === ZSTD_MAGIC[2] && raw[i + 3] === ZSTD_MAGIC[3]) return i
  }
  return -1
}
 ...
 * 为取 header 而整包解压大日志。
 */
function readHeaderLine(path: string, isZstd: boolean): string | undefined {
  let fd: number
  try {
    fd = openSync(path, 'r')
  } catch {
...
function parseLogHeader(line: string): { id: string; cwd?: string; parentSession?: string } | undefined {
...
export function parseLogEvents(text: string): SessionEventLike[] {
  ...
  for (const line of text.split('\n')) {
    ...
    events.push({
      ...
      ...parsed.data !== undefined && parsed.data !== null ? { data: parsed.data as SessionEventLike['data'] } : {},
    })
  }
  return events
}

/* ── 会话 header 索引（避免每次 cost op 全目录重读 header）────────── */
 ...
 */
const projectIndexCache = new Map<string, { dirKey: string; at: number; byId: Map<string, SessionIndexEntry> }>()
const PROJECT_INDEX_TTL_MS = 60_000

/** 取一个项目目录的会话索引（id → 日志位置 + header 血缘信息）。 */
function projectSessionIndex(projectDirPath: string): Map<string, SessionIndexEntry> {
  let names: string[]
  try {
 ...
 * 命中后整包解码。
 */
function readSessionLog(sessionId: string): SessionLike | undefined {
 ...
 */
function aggregateTodayFile(
  sample: FileSample,
  todayStart: number,
  config: PriceConfig,
  providerBaseUrls: Record<string, string>,
): FileTodaySample {
  const out: FileTodaySample = { byProvider: new Map() }
  if (sample.cwd !== undefined) out.cwd = sample.cwd
