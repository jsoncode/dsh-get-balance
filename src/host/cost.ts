  ...
  for (const project of projects) {
    ...
    for (const name of names) {
      ...
      for (const candidate of candidates) {
        ...
        if (header === undefined || header.id !== sessionId) break
...
function foldDescendantSamples(sessionId: string, cwd: string | undefined): Map<string, StepSample> {
  ...
  const byId = new Map<string, { parentSession?: string; path: string; zstd: boolean }>()
  let names: string[]
  try {
    names = readdirSync(projectDirPath)
  } catch {
...
const todayFileCache = new Map<string, { mtimeMs: number; size: number; todayStart: number; configKey: string; sample: FileTodaySample }>()
...
function parseTodayFile(
  ...
  for (const line of text.split('\n')) {
    ...
    const data = parsed.data as { usage?: UsageLike } | undefined
  ...
  for (const project of projects) {
    ...
    for (const sessionId of sessionIds) {
      ...
      for (const candidate of candidates) {
        ...
        let sample: FileTodaySample | undefined
