/**
 * dsh-get-balance —— tsdown 构建配置（与 dsh-jenkins 相同的 client bundle 方案）。
 *
 * 两个产物：
 * - lib/index.js  宿主半边（Node，ESM）：src/host/index.ts 打包，@deepseek-ai/* 保持 external；
 * - lib/client.js 浏览器半边（CJS）：src/client/index.ts 打包为单文件 CJS 工厂，
 *   通过 banner/intro/footer 生成 window.__ModuleLoader__.load({ id, factory }) 包装；
...
const clientConfig: UserConfig = {
  ...
  // __ModuleLoader__ 工厂包装：产物整体位于 factory 函数体内，
  // 顶部 require(...) 解析到 factory 参数（seed 表）。
  banner: `window.__ModuleLoader__.load({ id: 'dsh-get-balance', factory: (require) => {
